...
for sc=1:columns(laserEndPntsMapFrame)
        ...
        [_, _, _, X, Y] = bresenham2(map, [robPoseMapFrame'; laserEndPntsMapFrame(:, sc)'], 0);

        %TODO: add them to freeCells
        freeCells = [freeCells, [Y; X]];

endfor
%TODO: update the log odds values in mapUpdate for each free cell according to probFree.
freeIndices = sub2ind(size(mapUpdate), freeCells(1, :), freeCells(2, :));
mapUpdate(freeIndices) += prob_to_log_odds(probFree);

%TODO: update the log odds values in mapUpdate for each laser endpoint according to probOcc.
occIndices = sub2ind(size(mapUpdate), laserEndPntsMapFrame(1, :), laserEndPntsMapFrame(2, :));
mapUpdate(occIndices) += prob_to_log_odds(probOcc);
