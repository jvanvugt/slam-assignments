...
sigma_points(:, 1) = mu;
root_term = chol((n+lambda) * sigma);

sigma_points = horzcat(mu, mu+root_term, mu-root_term);

% TODO compute weight vectors w_m and w_c
w_m_c_i = repmat(1 / (2*(n+lambda)), 1, 2*n);
w_m = [lambda / (n+lambda), w_m_c_i];
w_c = [w_m(1) + (1 - alpha*alpha + beta)];
end
