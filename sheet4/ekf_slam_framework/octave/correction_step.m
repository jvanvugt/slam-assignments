...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
	    ...
	    mu(2*landmarkId+2:2*landmarkId+3) = mu(1:2) + [z(i).range*cos(z(i).bearing+mu(3)),
    ...
    d = mu(2*landmarkId+2:2*landmarkId+3) - mu(1:2);
    q = d' * d;
    sq = sqrt(q);
    expectedZ(2*i-1:2*i) = [sq, atan2(d(2), d(1)) - mu(3)];
	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	low_Hi = (1/q) * [-sq*d(1), -sq*d(2),  0, sq*d(1), sq*d(2);
                          d(2),    -d(1), -q,   -d(2),    d(1)];
